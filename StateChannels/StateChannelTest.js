...
async function genSig(args, address) {
    ...
    let sign = await web3.eth.sign(hash,address,true);
    r = sign.slice(0, 64);
    s = sign.slice(64, 128);
    v = sign.slice(128, 130);
    return [r,s,v];
}
...
async function deployState(gameaddress, game) {
    ...
    cntr.send(
        ...
        state_cntr = newContractInstance;   
    ...
    var events = game.events.allEvents({fromBlock: 0, toBlock: 'latest'},
        ...
        if (!error) {
            ...
            if(event.event == "Event_Initialized"){
                ...
                */
                resp = await game.methods.applyStateInit(web3.utils.toHex(hash_a), web3.utils.toHex(hash_b), value_a, value_b,
                    state, lock_address, _aliceValue, _bobValue, _counter);
                ...
                called("applyStateInit", snd.gasUsed);

            }else if(event.event == "Event_State_Applied"){
                console.log("State applied with alice: "+
                    event.returnValues.aliceValue+" wei, bob: "+event.returnValues.bobValue + 
                    " channel time" + event.returnValues.time);
                var _hash = event.returnValues.hash;
                console.log("adsfasdf:" + _hash);
                resp = await game.methods.applyStateFinalize(
                    _hash, sig_b[0], sig_b[1], "0x"+sig_b[2]);
                ...
                console.log("Alice updates the contract on the blockchain_2");
                called("applyStateFinal", snd.gasUsed);

            }else if(event.event == "Event_State_Finalized"){
                console.log("State finalized: "+event.returnValues.counter);
                ...
                await sleep(10000);
                resp = await state_cntr.methods.proposeStopGame();
